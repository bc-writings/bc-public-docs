% !TEX TS-program = lualatex

%%%
% src::
%     url = https://github.com/pfradin/luadraw/discussions/148#discussioncomment-14976053
%%%

\documentclass[border = 3pt]{standalone}

%%%
% Des couleurs faciles d'emploi via le package \xcolor!
%%%
\usepackage[svgnames]{xcolor}

%%%
% La bibliothèque \luadraw allie une facilité d’utilisation
% à un rendu particulièrement soigné.
%%%
\usepackage[3d]{luadraw}

\begin{document}

\begin{luadraw*}{name = central-proj-of-cube-box-coord}
local polyext = require 'luadraw_polyhedrons'

local ld = luadraw

local M    = ld.pt3d.M
local vecI = ld.pt3d.vecI
local vecJ = ld.pt3d.vecJ
local vecK = ld.pt3d.vecK

------
-- Nos ¨elts de base.
------
local c_len = 2   -- Côté du cube.

local z0    = -3  -- Niveau du sol de projection.
local light = M(-5, -5, 5)

local proj = function(A)
  return ld.proj3dO(
    A,                -- Ce que l'on projette.
    {z0*vecK, vecK},  -- Plan de projection via un point
                      -- et un vecteur normal.
    A - light         -- Direction.
  )
end

------
-- ¨Def de la zone graphique.
------
local graphview = ld.graph3d:new{
  window3d = {-5, 8, -5, 8, -3, 5},
  adjust2d = true,
  bbox     = false,
  viewdir  = {30, 60},
  size     = {10, 10}
}

------
-- Réglages liés aux lignes.
------
graphview:Linewidth(12)

------
-- Un cube, un faisceau et une ombre.
------
local C = ld.parallelep(
  M(-c_len, -c_len, -c_len)/2,
  c_len*vecI, c_len*vecJ, c_len*vecK
)

C = ld.rotate3d(
  C,
  25,
  {
    M(c_len/2, -c_len/2, -c_len/2),  -- Point de l'axe de rotation.
    M(-1, 1, 1)                      -- Vecteur dirigeant l'axe de
                                     -- rotation.
  }
)

------
-- Comme nous savons que l'ombre est convexe, une seule facette
-- est renvoyée par ''cvx_hull3d''.
------
local shade = ld.cvx_hull3d(
  ld.ftransform3d(
    C.vertices,
    proj
  )
)[1]

------
-- Construction du faisceau comme conséquence de la source
-- lumineuse et de l'ombre...
------
local beam = ld.pyramid(shade, light, true)

------
-- Il ne reste plus qu'à dessiner.
------
graphview:Dboxaxes3d({
  grid      = true,
  gridcolor = "gray",
  fillcolor = "lightgray!40"
})

graphview:Dballdots3d(light, "Yellow", 5)

graphview:Dpolyline3d(
  shade,
  true,
  "draw = none, fill = DarkGray, fill opacity = 0.9"
)

graphview:Dpoly(
  C,
  {
    color = "cyan",
    opacity = 0.8
  }
)

graphview:Dpoly(
  beam,
  {
    color    = "yellow",
    opacity  = 0.3,
    contrast = 0.5,
    mode     = ld.mShadedOnly
  }
)

graphview:Linewidth(6)

for _, A in ipairs(shade) do
  graphview:Dseg3d(
    {light, A},
    "dotted, yellow"
  )
end

------
-- Montrons le résultat de notre oeuvre.
------
graphview:Show()
\end{luadraw*}

\end{document}
