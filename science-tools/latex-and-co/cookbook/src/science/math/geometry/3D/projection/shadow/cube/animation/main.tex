...
local C = polyext.cube(center, M(-1, -1, z0))
...
local theta = ld.linspace(0, 360, NB_IMAGES + 1)

function makeframe(k)
  graphview:Defaultattr()  -- IMPORTANT!
  ...
  shadow = ld.cvx_hull3d(
    ld.ftransform3d(C_rot.vertices, proj)
  )[1]

  if not graphview:Isvisible(shadow) then
    shadow = ld.reverse(shadow)
  end

  beam = ld.pyramid(shadow, light, true)
...
\def\nb{\directlua{tex.print(NB_IMAGES)}}
