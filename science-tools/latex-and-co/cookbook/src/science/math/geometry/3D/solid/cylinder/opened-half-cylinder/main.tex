...
--              ``{A, n}``.
------
function ld.graph3d:addHalfCyl(A, r, B, n, options)
  local cyl  = ld.cylinder(A, r, B, 150)
  local P, w = {A, n}, B - A

  cyl = ld.cutpoly(cyl, P)

  local L = self:Outline(cyl)
  L = ld.concat(L.visible, L.hidden)

  n = r * ld.pt3d.normalize(n)

  local B1, B2 = ld.rotate3d(B + n, -90, {B, w})
  local B2     = 2*B - B1
