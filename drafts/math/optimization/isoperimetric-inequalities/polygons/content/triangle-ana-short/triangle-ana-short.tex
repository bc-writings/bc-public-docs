...
\begin{proof}[\altproof{2}]
	Faisons appel à \textbf{l'analyse élémentaire}.
	Le fait \ref{tri-one-side-fixed} implique de chercher un éventuel triangle \og optimal \fg\ parmi les triangles isocèles.
	Soit donc $ABC$ un triangle isocèle en $C$. 
	Plaçons-nous dans un repère orthonormé $\pvaxes{O | i | j}$ tel que  $A\coord{0 | 0}$, $B\coord{AB | 0}$ et $C\coord{x_C | y_C}$ avec $y_C \geq 0$, et posons $c = AC = BC \neq 0$ et $s = \frac{p}{2}$.
	Nous avons 
	$x_B = 2 s - 2 c \neq 0$, et 
	...
	Ceci prouve que $ABC$ équilatéral est la solution \og optimale \fg.
\end{proof}
...
\begin{proof}[\altproof{3}] \label{tri-topo-comp}
	Utilisons \textbf{la continuité et la compacité sans le théorème des extrema liés}.% (nous généraliserons cette idée au cas des polygones à $n$ côtés).
	%
	\begin{itemize}
