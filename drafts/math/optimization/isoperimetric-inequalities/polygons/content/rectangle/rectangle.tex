...
\begin{remark} \label{ineq-geo-quad-arith}
	...
	%
	Pour clore cette remarque, notons que 
	$\sqrt[3]{L \ell h} \leq \frac13 (L + \ell + h)$ s'obtient via les figures suivantes qui donnent
	$(L + \ell + h)^3 \geq 27 L \ell h$ où $27 = 3^3$, les pavés étant tous de dimension $L \times \ell \times h$.

	\begin{center}
		\small\itshape
		\includegraphics[scale=.25]{content/rectangle/pave-2-cube-1.png}
		
		\smallskip
		La brique de base.
		
		\includegraphics[scale=.25]{content/rectangle/pave-2-cube-2.png}
		
		\smallskip
		Un 1\ier\ niveau à empiler $3$ fois.
	\end{center}
\end{remark}
