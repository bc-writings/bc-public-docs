...
\begin{multicols}{3}
	...
	\begin{center}
		\foreach \i in {1,...,5} {
			...
			\includegraphics[scale=.4]{content/polygon/sufficient-cond/g-area-why-step-\i.png}
...
\begin{proof}
	...
	\explnext*{La suite $A^{\,\prime}$ est $n$-périodique.}{}
		\mu_{1 \to n}^M (\setproba{L})
		+
		\dsum_{i=1}^{n} \det \big( \vect{\Omega M} , \vect{M A^{\,\prime}_{i}} \big)
		-
		\dsum_{i=1}^{n} \det \big( \vect{\Omega M} , \vect{M A^{\,\prime}_i} \big)
	\explnext{}
		\mu_{1 \to n}^M (\setproba{L})
...
\begin{proof}
	...
	\explnext*{$A^{\,\prime}_0 = A^{\,\prime}_n$}{}
