...
\begin{proof}
	...
	\begin{itemize}
		...
		\noindent
		Si l'on connait le lien entre déterminant et produit vectoriel, il n'y a rien à faire. Pour les autres, il existe une méthode élégamment brutale: par une rotation directe, qui ne change pas le signe du déterminant, on se ramène au cas où $A\coord{0|0}$, $B\coord{AB|0}$ et $C\coord{x_C | y_C}$, de sorte que $\det \big( \vect{AB} , \vect{AC} \big) = AB \cdot y_C$.
