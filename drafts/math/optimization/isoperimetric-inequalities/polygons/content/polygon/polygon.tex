...
\begin{proof}
	...
	%
	\begin{enumerate}
		\item Si $m = 0$, il n'y a rien à faire. 

		\item \label{add-vertex-start}
		Sinon, considérons $[AB]$ un côté quelconque de $\setproba{C}$.
	...
	%	
		...
		Comme $HC$ peut être rendu aussi proche de $0$ que souhaité, il est aisé de voir que l'on peut choisir cette distance de sorte que $AC + BC < AB + \frac{\delta}{m}$, de sorte que le périmètre augmente de $\frac{\delta}{m}$.

		\item En répétant $(m-1)$ fois les étapes \ref{add-vertex-start} à \ref{add-vertex-end} avec le nouveau \ngone\ convexe $\setproba{C}^{\,\prime}$, mais sans changer la valeur de $m$, nous avons un \ngone\ convexe $\setproba{P}^{\,\prime}$ tel que
		$\area{\setproba{P}^{\,\prime}} > \area{\setproba{P}}$
		et
