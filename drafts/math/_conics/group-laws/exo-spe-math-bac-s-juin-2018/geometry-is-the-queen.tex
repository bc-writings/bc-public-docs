...
\medskip
Un peu de conjecture pour commencer... Dans les représentations ci-dessous, nous partons de deux points $M(x ; y)$ et $N(x' ; y')$ sur $\setgeo{H}$,
puis nous ajoutons $E(1 ; 0)$, cf. le neutre de $\star$, et $P(x'' ; y'')$
où
$\begin{pmatrix} 
  x'' \\ 
  y''
...
\begin{pmatrix}
  ...
  x y' + x' y 
\end{pmatrix}$,
ces derniers points étant aussi sur $\setgeo{H}$.
...
\begin{enumerate}
	\item Si $M \neq N$ et $x_M \neq x_N$, alors $P$ est le second point d'intersection de $\setgeo{H}$ avec la parallèle à $(MN)$ passant par $E$.


	\item Si $M \neq N$ et $x_M = x_N$, alors $P = E$ (ce second cas peut être vu comme un cas limite du précédent avec un point d'intersection \squote{double}).


	\item Si $M = N$, alors $P$ est le \squote{second} point d'intersection de $\setgeo{H}$ avec la parallèle de la tangente à $\setgeo{H}$ au point $M$.
	Notons que si $M$ et $N$ ont pour coordonnées $(-1 :0)$, alors $P = E$ est un point d'intersection double.
\end{enumerate}
...
% --------------- %
	...
	$\vect{u} \big( - \pder{F}{Y}{1}(x ; y) ; \pder{F}{X}{1}(x ; y) \big)$,
	c'est-à-dire
	$\vect{u} ( 16y ; 2x )$
