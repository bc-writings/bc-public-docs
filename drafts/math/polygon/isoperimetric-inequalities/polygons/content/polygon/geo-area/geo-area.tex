...
\begin{multicols}{2}
	...
	\includegraphics[scale=.3]{content/polygon/geo-area/geo-area-ncycle-not-opti-pb-1.png}

	\includegraphics[scale=.3]{content/polygon/geo-area/geo-area-ncycle-not-opti-pb-2.png}
\end{multicols}
...
\begin{center}
	\small\itshape
	\includegraphics[scale=.25]{content/polygon/geo-area/geo-area-odd-even.png}
...
\begin{proof}
    ...
    \begin{multicols}{3}
        ...
        \begin{center}
            \includegraphics[scale=.4]{content/polygon/geo-area/triangulation-3.png}
    ...
    \begin{multicols}{2}
        \small\itshape
    	\begin{center}
        	\includegraphics[scale=.4]{content/polygon/geo-area/mini-ear-1.png}
    	...
    	\begin{center}
        	\includegraphics[scale=.4]{content/polygon/geo-area/mini-ear-2.png}
	...
	\begin{itemize}
	    ...
	    \begin{multicols}{2}
    	    \small\itshape
    		\begin{center}
        	\includegraphics[scale=.4]{content/polygon/geo-area/triangulation-proof-OK.png}
    	...
    	\end{center}
	    	...
	    	\begin{center}
        	\includegraphics[scale=.4]{content/polygon/geo-area/triangulation-proof-KO.png}
...
\begin{fact} \label{garea-cont}
    ...
    On note $\setproba{U} \subset \RR^{2n}$ l'ensemble des uplets de coordonnées $\big( x(A_1) ; y(A_1) ; \dots ; x(A_n) ; y(A_n) \big)$ où $A_1 A_2 \cdots A_n$ désigne un \ncycle\ de longueur $\ell$.
   	%
	Nous avons la continuité de la fonction $\alpha: \setproba{U} \rightarrow \RRp$ qui à un uplet de $\setproba{U}$ associe l'aire géométrique du \ncycle\ qu'il représente.
\end{fact}


\begin{proof}
	Notons $\dcup_{i} \setproba{P}_i$ la frontière de la surface impaire de $\setproba{L}$, et $\setproba{P}_i = A_{i,1} A_{i,2} \cdots A_{i,n_i}$, les $A_{i,j}$ pouvant désigner des intersections d'arêtes non contigües.
    %
    \begin{itemize}
        \item Par définition, 
        $\geoarea{\setproba{L}} = \dsum_{i} \area{\setproba{P}_i}$.
        Si $\dcup_{i} \setproba{P}_i = \emptyset$, alors 
		$\geoarea{\setproba{L}} = 0$,
		sinon
		$ \geoarea{\setproba{L}} 
		= \frac12 \dsum_{i} \big\lvert
			...
			\big)
		 \big\rvert$
	    selon le fait \ref{garea-ngone}.%
	    \footnote{
	    	Il suffit d'utiliser l'origine $\mathrm{O}$ du repère comme point de calcul.
		}
