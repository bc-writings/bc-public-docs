...
\begin{proof}
    ...
    Pour finir, justifions que $\setproba{M} \in \setproba{U}$ maximisant $\alpha$ est forcément un \ngone\ convexe.
    %
    \begin{itemize}
		\item Les côtés de $\setproba{M}$ contiennent tous exactement deux sommets.
		...
		\item La positivité des déterminants $\det \big( \vect{\primeit{A}_i \primeit{A}_{i+1}}, \vect{\primeit{A}_i \primeit{A}_k} \big)$ et le point précédent impliquent que deux côtés non contigus de $\setproba{M}$ ne peuvent pas être sécants.
    \end{itemize}
    ...
    fait \ref{tri-one-side-fixed} 
    
    Le fait \ref{conv-from-non-neg-det} donne un \kgone\ convexe $\setproba{C}$, où $k \leq n$, tel que
...
%		Nous savons que pour deux sommets consécutifs $\primeit{A}_i$ et $\primeit{A}_k$ de $\setproba{C}$, seuls les sommets $\primeit{A}_j$, pour $j \in \ZintervalC{i}{k}$, sont situés sur $[\primeit{A}_i \primeit{A}_k]$.
		...
		et
		$\sarea{\primeit{\setproba{C}}} \geq \sarea{\setproba{M}}$.
		%
		\begin{enumerate}
		    \item Si $M$ est déjà un \ngone\ convexe, alors $\setproba{C} = \setproba{M}$ et donc $\primeit{\setproba{C}} \eq[def] \setproba{C}$ convient.
		    

		    \item Sinon, forcément $\setproba{C}$ possède strictement moins de sommets que $\setproba{M}$, de sorte que $\cyclelen{\setproba{C}} < \cyclelen{\setproba{M}}$.
		    Dans ce cas, 
		    le \ngone\ convexe $\primeit{\setproba{C}}$ est donné par le fait \ref{bigger-convex} ci-dessous appliqué 
		    au convexe $\setproba{C}$ avec $s = n - k$ et $L = \cyclelen{\setproba{M}}$,
		    en se souvenant que 
...
\begin{proof}
    ...
    %
    Comme pour $\setproba{R}$ dans la preuve précédente,
	nous pouvons supposer $\setproba{P}$ d'origine $\pt{O}$, via une translation,
	puis nous avons
    ...
    Dès lors, pour conclure, il suffit de faire appel au fait \ref{at-least-one-ncycle}.    
\end{proof}
