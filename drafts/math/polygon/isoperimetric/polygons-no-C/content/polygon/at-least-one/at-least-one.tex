...
\begin{fact} \label{conv-from-non-neg-det}
	...
	\begin{enumerate}[label=\roman*.]
		...
		ceci se faisant en retirant des sommets de $\setproba{L}$, sans modifier l'ordre de parcours pour les sommets gardés.
    \end{enumerate}
\end{fact}
...
\begin{proof}
	...
	$\det \big( \vect{A^{\,\prime}_i A^{\,\prime}_{i+1}}, \vect{A^{\,\prime}_i A^{\,\prime}_k} \big) \geq 0$.
	%
    Traitons alors le cas où $\setproba{L}$ est non totalement dégénéré, de sorte que 
    $\exists (i, k) \in \ZintervalC{1}{n}^2$
	tel que
	$\det \big( \vect{A^{\,\prime}_i A^{\,\prime}_{i+1}}, \vect{A^{\,\prime}_i A^{\,\prime}_k} \big) > 0$.
...
\begin{proof}
     ...
     Donc, par continuité et compacité, $\alpha$ admet un maximum sur $\setproba{U}$, celui-ci étant positif strict pour les raisons suivantes.
    %
    \begin{itemize}
		\item Via une translation, on peut supposer $\setproba{R}$ d'origine $\pt{O}$.


        \item Un \nreg\ convexe $\setproba{R}$, de périmètre $\ell$, est d'aire non nulle.
		...
		Il en va de même pour $\setproba{R}^{\mathrm{op}}$.
    \end{itemize}
...
\begin{proof}
	...
	%
	\begin{itemize}
		\item Via une translation, on peut supposer $\setproba{P}$ d'origine $\pt{O}$.
		...
		Quitte à échanger $\setproba{P}$ et $\setproba{P}^{\mathrm{op}}$, on peut supposer que $\setproba{P} \in \setproba{U}$.
