...
\begin{remark}
    ...
    \begin{enumerate}[label=\fbox{\small\bfseries\textsf{A\kern.25pt\arabic*}}]
        ...
        \item Tant que $\primeit{A}_j = \primeit{A}_i$, on augmente $j$ de $1$.

        \item On pose $k = j + 1$.

        \item Tant que $\primeit{A}_k \in (\primeit{A}_i \primeit{A}_j)$, on augmente $k$ de $1$.
...
\begin{fact} \label{at-least-one-ncycle}
    ...
    %
    De plus, les sommets d'un \ncycle\ maximisant $\alpha$ sont distincts deux à deux.
\end{fact}
...
\begin{proof}
    ...
    Soit $\setproba{M} \in \setproba{U}$ maximisant $\alpha$, il reste à vérifier que les sommets de $\setproba{M}$ sont distincts deux à deux.
...
\begin{proof}
    ...
    Nous savons que les sommets de $\setproba{M}$ sont distincts deux à deux,
    et que $\sarea{\setproba{M}} > 0$, 
    d'où la non totale dégénérescence de $\setproba{M}$.
    Dès lors,
...
%		Nous savons que pour deux sommets consécutifs $\primeit{A}_i$ et $\primeit{A}_k$ de $\setproba{C}$, seuls les sommets $\primeit{A}_j$, pour $j \in \ZintervalC{i}{k}$, sont situés sur $[\primeit{A}_i \primeit{A}_k]$.
		...
		\item Le choix de $\setproba{M}$ implique 
...
Le fait suivant, nécessaire pour la preuve précédente, nous sera aussi utile pour obtenir qu'un \ngone\ \focus{maximal} doit être convexe.
...
\begin{proof}
	...
	\begin{enumerate}
		...
		nous obtenons un \xgone{(n+s)} convexe $\primeit{\setproba{K}}$ vérifiant à la fois
		$\area{\setproba{P}} < \area{\setproba{K}}$
		et
